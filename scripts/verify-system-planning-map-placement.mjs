...
expect(resolvedAmplifiers.length === 0, 'declared amplifier count still produced frontend fallback placements')
...
expect(layout.resolveLayoutAmplifiers(parsedCountOnlyLayout).length === 0, 'count-only layout still produced map placements')
...
expect(
  layout.resolveLayoutAmplifiers(parsedNestedSpansLayout).length === 0,
  'nested spans still inferred amplifier positions without explicit amplifier nodes',
)
...
expect(
  routeLengthFallbackAmplifiers.length === 0,
  'current route length still generated frontend amplifier placements',
)
...
expect(!dialogSource.includes('applyPlanningResult'), 'system planning still applies device placement in the frontend')
expect(dialogSource.includes("emit('apply-result'"), 'explicit backend results are not returned to the owning view')
...
expect(!dialogSource.includes('backendAmps'), 'dead backend-amplifier fallback branch still exists')
expect(!dialogSource.includes('rawBranches'), 'frontend branch reconstruction still exists')
expect(!dialogSource.includes('findDeviceByKp'), 'KP-tolerance endpoint matching still exists')
