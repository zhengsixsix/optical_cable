...
try {
  ...
  const autoTable = {
    ...basicOnlyTable,
    id: 'legacy-empty-table-id',
    name: 'Existing data RPL',
    autoGenerated: true,
  }
  const autoWorksheet = await readRplWorksheet(exportToExcel, autoTable)
  assert.equal(autoWorksheet.getCell('B7').value, 'Start')
  assert.equal(autoWorksheet.getCell('AD7').value, 100)
  assert.equal(autoWorksheet.getCell('AF7').value, 'must not reach industry columns')
  assert.equal(autoWorksheet.getCell('V8').value, 15)
  assert.equal(autoWorksheet.getCell('X8').value, 1)
  assert.equal(autoWorksheet.getCell('AE8').value, 2)
  assert.equal(autoWorksheet.getCell('W9').value, 15)
  assert.equal(autoWorksheet.getCell('AD9').value, 200)
